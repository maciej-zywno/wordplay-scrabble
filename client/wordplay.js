...
var set_selected_positions = function (word) {
  ...
  for (var pos = 0; pos < LETTER_COUNT; pos++) {
...
// arrays of array, outer arrays are rows
Template.board.board_rows = function (i) {
    var rows = [];
    var i, j;
    for (i = 0; i < SIZE; i += 1) {
        rows[i] = [];
        for (j = 0; j < SIZE; j += 1) {
            var index = i * SIZE + j
            rows[i][j] = [Template.board.selected(index), Template.board.square(index)];
        }
    }
    return rows;
}
...
Template.board.clock = function () {
  ...
  if (!clock || clock === 0) {
    return format_seconds(GAME_SECONDS);
  } else {
    return format_seconds(clock);
  }
};
